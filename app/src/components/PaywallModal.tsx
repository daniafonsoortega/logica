...
const PLANOS = [
  ...
  { id: 'anual',   label: 'Anual',   preco: 'R$89',    periodo: '/ano',    destaque: true,
...
export default function PaywallModal({ onClose, reason = 'daily_limit' }: Props) {
  ...
  async function handleCheckout(planoId: string) {
    setLoading(planoId)
    try {
      // Garante que o usuário está logado antes do checkout
          ...
          <div className="grid grid-cols-3 gap-2">
            {PLANOS.map(p => (
              <button
                ...
                className={`relative rounded-xl border-2 p-3 text-center transition-all disabled:opacity-60 ${
                  p.destaque
              ...
              >
                {p.destaque && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap">
                    {p.economia}
                  </span>
                )}
