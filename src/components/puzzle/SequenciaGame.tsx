...
export default function SequenciaGame({ puzzle }: Props) {
  ...
  function verificar() {
    ...
    if (correto) {
      ...
      salvar(puzzle.id, true, pistasVis || dicasVistas > 0, Math.round((Date.now() - startTime.current) / 1000))
      ...
      salvar(puzzle.id, false, pistasVis || dicasVistas > 0, Math.round((Date.now() - startTime.current) / 1000))
