...
export async function GET() {
  ...
  const [profiles, subs, couponUses, coupons] = await Promise.all([
    supabaseAdmin.schema('logicamente').from('profiles').select('id, email, is_premium, plano, created_at'),
    supabaseAdmin.schema('logicamente').from('subscriptions').select('id, plano, status, created_at'),
    supabaseAdmin.schema('logicamente').from('coupon_uses').select('*').order('created_at', { ascending: false }),
    supabaseAdmin.schema('logicamente').from('coupons').select('*').order('created_at', { ascending: false }),
  ])
