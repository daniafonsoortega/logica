...
export type TipoPuzzle = 'grade' | 'detetive' | 'sequencia' | 'mentiu' | 'codigo' | 'cifra'
...
// ── Puzzle Tipo 6: Cifra Simbólica ───────────────────────────────
export interface PuzzleCifra {
  id: string
  tipo: 'cifra'
  nivel: Nivel
  tema: string
  intro: string
  frase_original: string           // "LOGICA E PODER"
  frase_cifrada: string            // "△○□★◆● ● □○◆●◆"
  mapa_cifrado: Record<string, string>  // letra → símbolo
  solucao: Record<string, string>       // símbolo → letra
  revelados?: Record<string, string>    // símbolos já revelados como dica
  pistas: string[]
}

// ── Union type ────────────────────────────────────────────────────
export type Puzzle = PuzzleGrade | PuzzleDetetive | PuzzleSequencia | PuzzleMentiu | PuzzleCodigo | PuzzleCifra
