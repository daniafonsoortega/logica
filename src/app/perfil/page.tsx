...
import { Save, LogOut, Trophy, Flame, CheckCircle2, Calendar } from 'lucide-react'
...
export default function PerfilPage() {
  ...
  if (!user) return (
    <div className="max-w-md mx-auto py-24 text-center space-y-4">
      ...
      <a href="/" className="inline-block bg-blue-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-blue-700 transition-colors">
        Voltar ao início
      </a>
    </div>
  )
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              onChange={e => field('escolaridade')(e.target.value)}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {(profile.objetivo === 'Concurso Público' || profile.objetivo === 'OAB') && (
            <div className="space-y-1">
              ...
              <select
                value={profile.area_concurso ?? ''}
                onChange={e => field('area_concurso')(e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              >
                <option value="">Selecionar...</option>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              {NIVEL_OPTS.map(o => (
                <button
                  key={o}
                  onClick={() => field('nivel')(o.toLowerCase())}
          ...
          <div className="space-y-1">
            ...
            <select
              value={profile.frequencia ?? 'diaria'}
              onChange={e => field('frequencia')(e.target.value)}
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
            >
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
          <div>
            <p className="text-sm font-medium text-gray-700">Lembrete diário</p>
            <p className="text-xs text-gray-400">Receba uma notificação para treinar todo dia</p>
          </div>
          <button
            onClick={() => field('notificacoes')(!profile.notificacoes)}
            className={`relative w-11 h-6 rounded-full transition-colors ${profile.notificacoes ? 'bg-blue-600' : 'bg-gray-200'}`}
          >
