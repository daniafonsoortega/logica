...
export default function AuthButton() {
  ...
  function openModal(m: Mode) {
    setMode(m); setError(''); setEmail(''); setPassword(''); setModal(true)
  }
  ...
  async function handleEmail() {
    ...
    if (mode === 'login') {
      ...
      if (error) setError(error.message)
      else { setModal(false); window.location.href = '/perfil' }
    }
    setBusy(false)
  }

  async function signOut() {
    await supabase.auth.signOut(); setUser(null); setOpen(false)
  }
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <h2 className="text-xl font-black text-gray-900">{mode === 'login' ? 'Entrar' : 'Criar conta'}</h2>
            ...
            <p className="text-center text-xs text-gray-500">
              {mode === 'login'
                ? <>Não tem conta?{' '}<button onClick={() => { setMode('signup'); setError('') }} className="text-blue-600 hover:underline font-medium">Cadastrar</button></>
                : <>Já tem conta?{' '}<button onClick={() => { setMode('login'); setError('') }} className="text-blue-600 hover:underline font-medium">Entrar</button></>}
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
        ...
        <span className="hidden sm:block text-sm font-medium text-gray-700 max-w-[100px] truncate">{name}</span>
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute right-0 top-10 z-50 bg-white border border-gray-200 rounded-xl shadow-lg py-1 min-w-[180px]">
            <div className="px-4 py-2 text-xs text-gray-400 border-b border-gray-100 truncate">{user.email}</div>
            <a href="/perfil" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 font-medium">👤 Meu perfil</a>
            <a href="/ranking" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">🏆 Ranking</a>
